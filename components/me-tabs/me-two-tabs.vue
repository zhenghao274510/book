<template>
	<view class="row center" style="width: 750rpx;">
		<scroll-view class="scroll-h" scroll-x="true"  :scroll-into-view="scrollInto">
			<view v-for="(tab, index) in tabs" :key="tab.twoTypeId" class="uni-tab-item" :id="'a'+tab.twoTypeId" :data-current="index" @click="ontabtap(index)"
			 :class="tabIndex == index ? 'uni-tab-item-active' : ''">
				<text class="uni-tab-item-title" :class="tabIndex == index ? 'uni-tab-item-title-active' : ''">{{ tab.twoTypeName}}</text>
			</view>
		</scroll-view>
	</view>
</template>
...
<script>
	export default{
		...
		computed:{
			scrollInto(){
				let n=this.tabIndex;
				return 'a'+this.tabs[n].twoTypeId
			}
		},
		methods:{
			ontabtap(ind){
